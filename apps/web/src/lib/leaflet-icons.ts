...
export const pinIcon = L.divIcon({
  className: '',
  html: '<div style="background:var(--accent);width:16px;height:16px;border-radius:50%;border:3px solid var(--surface);box-shadow:0 1px 3px rgba(0,0,0,.4)"></div>',
  iconSize: [16, 16],
})
...
export function clusterIcon(count: number): L.DivIcon {
  ...
  return L.divIcon({
    className: '',
    html: `<div style="background:var(--accent);color:white;width:${size}px;height:${size}px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;border:3px solid var(--surface);box-shadow:0 1px 3px rgba(0,0,0,.4)">${count}</div>`,
    iconSize: [size, size],
  })
}
